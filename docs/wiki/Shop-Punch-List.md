...
| 4 | Legacy save/remove handlers survive ghost-tab only via lucky TclError — sweep to explicit window checks | Blueprint §11 trap | EXECUTED 2026-07-30 — finding: NO TclError luck existed (window withdraws, widgets live); `_study_workspace_visible()` one-seam guard on all 5 tab claims + add route; 3 gates. Awaiting owner UAT (yellow Save with workspace closed → honest terminal message) |
| 5 | Decline paths in `_ftb_action_*` chains: several return silently with neither breadcrumb nor status | red-plum class 2 | EXECUTED 2026-07-30 — 4 claimed-context swallows now breadcrumb + ⚠ status + stop the chain; 2 abnormal probes breadcrumbed; static decline-scan gate added. Awaiting owner UAT |
